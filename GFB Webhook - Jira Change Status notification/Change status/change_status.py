...
# CONFIG
# =========================
# 🔥 EDIT INI SESUAI PROJECT KAMU
JQL = "project = G4BQADB AND resolution = Fixed AND updated >= -24h ORDER BY updated DESC"

CACHE_FILE = BASE_DIR / "fixed_cache.json"
...
# =========================
def main():
    print("== BUG FIXED WATCHER STARTED ==")
    ...
    for issue in issues:
        ...
        # ✅ hanya ambil yang benar-benar Fixed
        if resolution != "Fixed":
            continue
    ...
    if messages:
        ...
        save_cache(cache)
        print(f"Sent {len(messages)} bug(s)")
    else:
        print("No new fixed bugs found")
